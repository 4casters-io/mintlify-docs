...
Emitted when a single side of a market changes — a new open offer, a fill, a cancel, or an edit. One emission per side that changed.
...
| `sideOrders` | array | Sorted open-offer depth on **just this side**. See schema below. |
...
This namespace emits exactly one event: **`positionUpdate`**. Every state change on one of your orders, and every fill that involves you (as offer creator or taker), arrives as a `positionUpdate`.
...
- **`unmatched`** — state of your **open offer**. Fields: `orderID`, `odds`, `offered` (original size), `filled` (cumulative amount filled into this order), `remaining` (`offered - filled`). Present whenever your open-offer state changed.
...
- `origin: "offer"` — `unmatched` is present (your open offer changed in some way).
- `origin: "wager"` — only `matched` is present, no `unmatched` (you took someone else's open offer).
...
| --- | --- | --- |
| **You created a new offer** | `origin === "offer"` AND `unmatched.filled === 0` AND `unmatched.remaining === unmatched.offered` AND `matched == null` | Your order is now open on the book. |
| **Someone partially took your offer** | `origin === "offer"` AND `unmatched.remaining > 0` AND `matched != null` | Use `matched.filled` for the size of this partial fill. The remainder is still open — you'll get another `positionUpdate` when it fills or is cancelled. |
...
| **You took someone else's offer** | `origin === "wager"` AND `matched != null` AND `unmatched == null` | This is the event you receive when you call `POST /session/v2/place` with `orderType !== "post"` and at least part of it crosses the book against another user's open offer. |
...
socket.on('positionUpdate', (raw) => {
  ...
  console.log('Created offer:', unmatched.orderID, '@', unmatched.odds, 'risk', unmatched.offered)
})
```

## REST cross-reference

For looking up a `txID` (matched fills) or `orderID` (your open-offer state), see [Look up single order](/pages/orders/lookup-order). To list all of your current open offers in one call, use [List my open orders](/pages/orders/list-open-orders).
