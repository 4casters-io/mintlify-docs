...
A maker-only `post` order on `POST /session/v2/place` that **would cross** the current market is **not** matched and **not** rested. There is no error for this — you receive **`HTTP 200`** with an **empty `matched` array and an empty `unmatched` object** (no `unmatched.orderID`).
...
- `matched` non-empty and/or `unmatched.orderID` present → your order took and/or rested.
